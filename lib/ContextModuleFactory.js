...
module.exports = class ContextModuleFactory extends Tapable {
	...
	create(data, callback) {
		...
		this.applyPluginsAsyncWaterfall("before-resolve", {
			...
			const queryIdx = resource.indexOf("?");
			if(queryIdx >= 0) {
				resourceQuery = resource.substr(queryIdx);
				resource = resource.substr(0, queryIdx);
			}
			...
			asyncLib.parallel([
				...
				module.applyPluginsAsyncWaterfall("after-resolve", {
					...
					return callback(null, new ContextModule(result.resolveDependencies, result.resource, result.resourceQuery, result.recursive, result.regExp, result.loaders, result.async, dependency.chunkName));
				});
			});
		});
	}

	resolveDependencies(fs, resource, resourceQuery, recursive, regExp, callback) {
		if(!regExp || !resource)
				...
				asyncLib.map(files.filter(function(p) {
					...
					fs.stat(subResource, (err, stat) => {
						...
						if(stat.isDirectory()) {
							...
							const request = "." + subResource.substr(resource.length).replace(/\\/g, "/");
							const obj = {
								context: resource,
								request: request
							};
